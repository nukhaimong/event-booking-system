...
export default function BookingForm({ event }: BookingFormProps) {
  ...
  const form = useForm({
    ...
    onSubmit: async ({ value }) => {
      ...
      try {
        ...
        if (response.error) {
          toast.error(response.error.message || 'Booking failed, please try again', { 
            id: toastId 
          });
          return;
        }
  ...
  const totalPrice = form.state.values.quantity * event.price;
    ...
    <div className="border-t pt-6">
      ...
      >
        ...
        <form.Field name="quantity">
          {(field) => (
            <div>
              <label className="text-sm font-medium block mb-1" htmlFor="quantity">
                Number of Tickets
              </label>
              <Input
                id="quantity"
        ...
        <div className="bg-gray-50 p-4 rounded-lg">
          ...
          <p className="text-sm text-gray-500 mt-1">
            ${event.price.toFixed(2)} × {form.state.values.quantity} ticket(s)
          </p>
        </div>
